...
```json
{
  "skill_name": "<kebab>",
  "evals": [
    { "id": 0, "name": "<kebab>", "prompt": "<string>", "expected_output": "<string>", "files": [] }
  ]
}
```
...
- Prefer fast-forward merges to keep linear history (precedent: PR #1, PR #2). For orphan-branch rewrites, merge with `-s ours` first so `main` becomes an ancestor.
